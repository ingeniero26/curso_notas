	...
	$.ajax({
		...
		// Swal.fire( 'Error', 'Usuario o clave incorrectos',  'warning');
			$.ajax({
				...
				alert(resp);
				 Swal.fire('Advertencia', 'Usuario o clave incorrectos: intentos fallidos '+(parseInt(resp)+1)+" Para iniciar sesión debe restablecer su contraseña",  'warning');
			})
			...
			$.ajax({
				...
				let timerInterval
						Swal.fire({
						  title: 'Bienvenido al sistema NOTAS 1.0',
...
// modificar datos usuario
    $('#tabla_usuario').on('click','.editar',function(){
		...
		$('#cmb_sexo_editar').val(data.cmb_sexo_editar).trigger("change");
		$('#cmb_rol_editar').val(data.rol_id).trigger("change");
		$('#txt_correo_editar').val(data.usuario_email);
    })
...
function Registrar_Usuario() {
	...
	if(usuario.length==0 || contra.length==0 ||  idrol.length ==0 
		|| 		correo.length==0) { 
		return Swal.fire( 'Mensaje de error',  'Digite los campos estan vacios', 'warning'
		);
	}
...
function TraerDatosUsuario() {
	...
	$.ajax({
		...
		if(data.length > 0) {
			...
			if(data[0][6]==="M"){
				$("#img_nav").attr("src","../plantilla/dist/img/avatar.png");
				$("#img_subnav").attr("src","../plantilla/dist/img/avatar.png");
				$("#img_lateral").attr("src","../plantilla/dist/img/avatar.png");
			} else {
				$("#img_nav").attr("src","../plantilla/dist/img/avatar.png");
				$("#img_subnav").attr("src","../plantilla/dist/img/avatar.png");
				$("#img_lateral").attr("src","../plantilla/dist/img/avatar.png");
